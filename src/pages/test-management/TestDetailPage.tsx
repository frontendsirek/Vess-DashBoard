...
import {
  buildFallbackTestDetail,
  buildTestDetailFromWizard,
  resolveTestDetailRecord,
  tests,
...
export default function TestDetailPage() {
  ...
  const detail = useMemo((): TestDetailRecord | null => {
    const wizard = (location.state as TestDetailFromWizardState | null)?.wizardResult
    if (wizard) return buildTestDetailFromWizard(wizard)

    const fromMock = resolveTestDetailRecord(testId)
    if (fromMock) return fromMock

    const record = tests.find((t) => t.id === testId)
    if (record) return buildFallbackTestDetail(record)
    return null
          ...
          <section className="flex flex-col gap-6 rounded-2xl border border-vess-grey-200 bg-vess-grey-50 px-4 py-5">
            ...
            <div className="flex flex-col gap-6">
              {chunkPairs(detail.configRows, 2).map((pair, rowIdx) => (
                <div key={`row-${rowIdx}`} className="flex flex-col gap-6 lg:flex-row lg:gap-6">
                  {pair.map((cell) => (
