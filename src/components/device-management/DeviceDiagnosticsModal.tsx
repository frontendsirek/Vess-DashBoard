    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent className="max-w-[560px] gap-0 border-0 bg-vess-grey-50 p-0 shadow-xl">
        <div className="flex flex-col gap-8 px-6 py-8">
          ...
          {isPending ?
            <p className="text-[15px] text-vess-grey-600">Running diagnostics…</p>
          : result ?
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <span
                  className={cn(
                    'inline-flex rounded-full px-3 py-1 text-[15px] font-normal leading-[18px]',
              ...
              <div className="flex flex-col gap-3">
                ...
                {result.issues.length === 0 ?
                  <p className="text-[15px] text-vess-green-500">No issues detected.</p>
                : <ul className="flex flex-col gap-2">
                    {result.issues.map((issue) => (
                      <li
                        key={issue}
                        className="rounded-lg bg-vess-grey-100 px-3 py-2 text-[15px] text-vess-grey-950"
                      >
                        {issue.replace(/_/g, ' ')}
                      </li>
                    ))}
              ...
              <p className="text-[13px] font-light text-vess-grey-500">
                Checked at {new Date(result.checked_at).toLocaleString()}
              </p>
            </div>
          : <p className="text-[15px] text-vess-grey-600">No diagnostics results yet.</p>}
...
function DiagnosticsMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-2 rounded-lg bg-vess-grey-100 px-4 py-3">
      <span className="text-[15px] font-light leading-[18px] text-vess-grey-950">{label}</span>
